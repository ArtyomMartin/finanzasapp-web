...
const PASO_FORMA = {
  titulo: "¿Cómo se ahorra?",
  contenido: (s, set) => (
    <div style={{ display: "flex", gap: "10px", marginBottom: "8px" }}>
      {[["unico", "Depósito único"], ["cuotas", "Varios depósitos"]].map(([f, label]) => (
        <button
          key={f}
...
const PASO_MODO_CUOTAS = {
  ...
  contenido: (s, set) => (
    <div style={{ display: "flex", gap: "10px", marginBottom: "8px" }}>
      {[["total", "Objetivo total"], ["cuota", "Objetivo mensual"]].map(([m, label]) => (
        <button
          key={m}
          ...
          style={{
            ...
            cursor: "pointer",
            fontWeight: "bold",
            backgroundColor: s.modoCuotas === m ? COLORES.positivo : COLORES.fondoOpcion,
    ...
    contenido: (s, set) => {
      ...
      return (
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          ...
          <input
            placeholder="Ej: Ahorro para un teléfono, reponer una campera..."
            value={s.nombre}
          ...
          {s.forma === "unico" || (s.forma === "cuotas" && s.modoCuotas === "total") ? (
            <>
              <label style={estilos.estiloLabel}>Objetivo de ahorro total</label>
              <input
                placeholder="0.00"
                type="number"
                value={s.monto}
            ...
            <>
              <label style={estilos.estiloLabel}>Lo que ahorro cada mes</label>
              <input
                placeholder="0.00"
                type="number"
                value={s.montoCuota}
          ...
          {s.forma === "cuotas" && (
            <>
              <label style={estilos.estiloLabel}>Cantidad de meses para ahorrar</label>
              <input
                placeholder="Ej: 6"
                type="number"
                value={s.numCuotas}
...
export default function Reposicion() {
  ...
  function wizardInicial(editando = null) {
    if (editando) {
      return {
        ...
        monto: String(editando.montoTotal),
  ...
  const fondosPendientes = (datos.ajustes || []).filter(a => {
    if (a.eliminado || a.tipo !== "fondo") return false
  ...
  const totalPendiente = fondosPendientes.reduce((acc, a) => {
    if (a.forma === "unico") {
  ...
  function guardar(s) {
    ...
    if (s.forma === "unico") {
      entrada = {
        id, usuarioId: s.usuarioId, tipo: "fondo", forma: "unico",
      ...
      entrada = {
        id, usuarioId: s.usuarioId, tipo: "fondo", forma: "cuotas",
        ...
        {fondosPendientes.length > 0 && (
          <div>
            <div style={{
              ...
              <span style={{ fontSize: "20px", fontWeight: "700", color: COLORES.positivo }}>
                {fmt(totalPendiente)}
              </span>
            </div>

            <p style={{ fontSize: "13px", color: COLORES.textoSecundario, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "12px" }}>
              Fondos / Reposiciones pendientes
            </p>

            {fondosPendientes.map(a => {
              ...
              const mesTexto = a.forma === "unico"
                ...
                : `${NOMBRES_MESES[a.mesInicio - 1]} ${a.anioInicio} · ${a.numCuotas} meses`
                ...
                <div key={a.id} style={{ marginBottom: "8px" }}>
                  <button
                    ...
                    style={{
                      ...
                      border: `1px solid ${activo ? COLORES.positivo : COLORES.borde}`,
      ...
      {confirmarEliminarId && (() => {
        ...
        return createPortal(
          ...
          >
            ...
            >
              <p style={{ fontSize: "18px", fontWeight: "700", color: COLORES.texto, marginBottom: "8px" }}>¿Eliminar fondo/reposición?</p>
